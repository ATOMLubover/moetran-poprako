...
- [ ] 项目翻译页面（左侧为图片显示区，可以缩放当前页漫画图片、添加/删除翻译源的标记（标记只是一个框框），右边为 sidebar，显示当前页上有的源的列表，如果无翻译则 text 显示为空，有翻译则显示翻译文本，每个翻译源 item 以颜色区分空、翻译完成、校对完成；点击某个翻译源的标记或者列表 item 后，会以一个悬浮窗的形式显示输入区，输入翻译后会自动同步到列表 item 中，悬浮窗可以拖动到任意位置）
